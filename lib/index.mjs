...
const DEFAULT_MAX_BYTES = 2e3;
/** Direct request (domestic target). */
function directRequest(u, headers, timeoutMs) {
	return new Promise((resolve, reject) => {
		const req = (u.protocol === "https:" ? https : http).request({
			...
			headers: Object.assign({
				"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) fetch-url",
				Connection: "close"
			}, headers)
		}, (res) => {
			const chunks = [];
...
*/
function proxiedRequest(u, headers, timeoutMs, proxyHost, proxyPort) {
	return new Promise((resolve, reject) => {
		...
		proxyReq.on("data", (d) => {
			...
			if (!/^HTTP\/1\.[01] 200/.test(head)) {
				reject(/* @__PURE__ */ new Error(`proxy CONNECT failed: ${head.split("\r\n")[0]}`));
				proxyReq.destroy();
				return;
			}
			tunnelUp = true;
		...
		proxyReq.setTimeout(timeoutMs, () => {
			proxyReq.destroy(/* @__PURE__ */ new Error("proxy connect timeout"));
		});
	});
}
...
function summarize(body, maxBytes) {
...
async function fetchUrl(args, exec) {
	let u;
	try {
		u = new URL(args.url);
	} catch {
	...
	if (u.protocol !== "http:" && u.protocol !== "https:") return {
		ok: false,
		error: `unsupported protocol: ${u.protocol} (only http/https)`
	};
	const timeoutMs = DEFAULT_TIMEOUT_MS;
	const maxBytes = args.maxBytes !== void 0 && args.maxBytes > 0 ? args.maxBytes : DEFAULT_MAX_BYTES;
	const headers = {};
	for (const [k, v] of Object.entries(args.headers ?? {})) if (typeof v === "string") headers[k] = v;
	...
	try {
		...
		} else res = await withAbort(directRequest(u, headers, timeoutMs));
...
function apply(ctx) {
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			proxy: {
				type: "string",
				description: "HTTP proxy \"host:port\" for overseas targets (e.g. \"127.0.0.1:7890\"). Omit for direct connection."
			},
			headers: {
				type: "object",
				additionalProperties: true,
				description: "Extra request headers as string values (e.g. {\"Accept\": \"application/vnd.github+json\"})."
			},
			maxBytes: {
				type: "number",
				description: `Max response characters to return (default ${DEFAULT_MAX_BYTES}).`
			},
			outFile: {
				type: "string",
				description: "Optional local file path to write the full response body to."
			}
		},
		output: {
			schema: {
			...
			render: (_args, value) => {
				if (!value.ok) return [{
					type: "text",
					text: `fetch failed: ${value.error ?? "unknown error"}`
				}];
