...
> `@deepseek-ai/dsh-fetch-url` 插件——注册一个模型可调用的 `fetch_url` 工具
...
```json
{
  "name": "@deepseek-ai/dsh-fetch-url",
  "version": "0.1.0",
...
const PLUGIN_ID = '@deepseek-ai/dsh-fetch-url'
...
# Install with: dsh plugin --profile <name> add @deepseek-ai/dsh-fetch-url
- insert:
    - id: fetch-url
      name: '@deepseek-ai/dsh-fetch-url'
```
...
1. **注册为 profile 依赖 + bundle**：编辑 `$profilePkg`，
   - `dependencies` 增加：`"@deepseek-ai/dsh-fetch-url": "link:E:/Projects/DSH/plugins/dsh-fetch-url"`
   - `dsh.profile.bundles` 数组增加：`"@deepseek-ai/dsh-fetch-url"`
